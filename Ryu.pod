...
=head1 SYNOPSIS
   ...
   1.0 of Math::Ryu. From then, it's the usual "goto" function to use.
   Some (hardly useful) functions from the earlier versions have been
   removed as of version 1.0. See the "FUNCTIONS" documentation below
   for a full list of current functions.
...
=head1 FUNCTIONS
   ...
   $str = nv2s($nv);
    ...
    value held by $nv. $str, when correctly assigned back to a perl number,
    will always be equivalent to $nv. Also, $str will contain as few digits
    as possible, without compromising that accuracy.
    Arguments that are not an NV will be coerced to NV.
  ...
  $str = q2s($nv);  # Use only when $Config{nvtype} is either
                    # '__float128' orIEEE 754 'long double'.
  ...
  $str = fmtpy($s);
   ...
   $nv = s2d($str); # string to double
    BE WARNED !! This function might not handle infinity correctly.
    Avoid it, especially if non-finite values might be necountered.
    ...
    string ($str) to a floating point value ($nv) and returns it.
