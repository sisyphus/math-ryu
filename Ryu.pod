...
=head1 DEPENDENCIES
   ...
   L<https://github.com/ulfjack/ryu/tree/master/ryu>
...
=head1 FUNCTIONS
   ...
   $str = nv2s($nv);
    ...
    There is no need for the caller to know the type ($Config{nvtype})
    of $nv - the nv2s() function handles that.
    ...
    Note: nv2s() spends some time reformatting the string that the ryu
          ...
          string, then save some time by avoiding the reformatting and
          instead run d2s() or ld2s() or q2s() - whichever is appropriate.
          See the d2s/ld2s/q2s documentation (below) to determine which
          is appropriate for your perl.
          ...
          You are, of course, free to employ your own reformatting sub.
   ...
   $str = n2s($sv);
    ...
    value of $sv considered in numeric context.
    For example:
     n2s('hello world') will return '0.0', not 'hello world';
     n2s('0.10000000000000001') will return '0.1' if nvtype is 'double'.
 ...
 $str = d2s($nv);  # Use only when $Config{nvtype} is 'double'.
  ...
  $str = fx2s($nv);
    Cast $nv to a single precision float value.
    Returns a decimal string representation of that single precision
    float value in the format 'mantissaEexponent' - eg:
      '1E-1' (for 0.1) and
      '1.4142135E0' (for square root of 2).
  ...
  $str = fmtpy($s);
    ...
    NOTE: If it's appropriate to do'fmtpy(d2s($nv))' then that's
          the same as doing 'nv2s($nv))'.
          If it's appropriate to do'fmtpy(ld2s($nv))' then that's
          the same as doing 'nv2s($nv))'.
          If it's appropriate to do'fmtpy(q2s($nv))' then that's
          the same as doing 'nv2s($nv))'.
  ...
  $bool = ryu_lln($sv);
   ...
   It does NOT handle MAGIC and is therefore NOT identical to
...
=head1 PRINT FUNCTIONS
  ...
  sany(@args); # "s"ay   "any"
   For each of the arguments ($_) in @args, both functions simply do a
   "print $_;" if the perl API function looks_like_number($_) returns
   true && either:
     a) the POK flag for $_ is set;
     or
     b) the IOK flag for $_ is set.

   If looks_like_number($_) returns true but both the POK and
   ...
   NOTE: If perl's NV precision is less than IV precision (as is the most
         common perl configuration) then pany() can produce a result
         that differs from nv2s(). Eg: pany(6.886264645392433e+16) will,
         in accordance with this documentation, output 68862646453924328.
         But nv2s(6.886264645392433e+16) will, in accordance with its
         documentation return the float string 6.886264645392433e+16.
         Aside from that, "pany($nv);" and "print nv2s($nv);" will
         produce identical outputs - assuming $nv is an NV, of course.

   For all other arguments (ie those for which looks_like_number($arg)
   has returned false), pany() will simply "print $_;".

   After sany() has printed the final arg, a terminating "\n" is printed.
   Otherwise sany() and pany() are identical.
   We should find that pany() and sany() produce outputs that are
   identical to print()/say() - except for those NV values for which
   perl's interpolation differs from the string returned by nv2s().
   ...
   by perl then you'll need to use print/say, not pany/sany.
...
=head1 LICENSE
   ...
   This perl module is Copyright 2024-25 Sisyphus
=head1 AUTHOR
