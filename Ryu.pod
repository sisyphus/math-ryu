...
=head1 FUNCTIONS
   ...
   $str = nv2s($nv);
    I think n2s() and nv2s() probably produce the same result.
...
=head1 PRINT FUNCTIONS
  ...
  sany(@args); # "s"ay   "any"
   Respectively:
   calls pn($arg) for args that look like a number - else calls
   "print $arg";
   and
   calls sn($arg) for args that look lije a number - else calls
   the equivalent of "say $arg".
   These last 2 functions aim to DWYM, irrespective of the type of
   argument(s) passed to them.
   They print out nv2s($arg) for arguments that are non-zero NVs.
...
=head1 SPRINTF FUNCTION
  ...
  $str = spanyf(@args);
   ...
   The given arguments should be enclosed in brackets.
