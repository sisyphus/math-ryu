...
=head1 PRINT FUNCTIONS
  ...
  Just some shorthand ways of printing out values.
  ...
  sany($sv);
  Respectively:
   calls pn($sv) if $sv is a number - else calls "print $sv";
   and
   calls sn($sv) if $sv is a number - else calls the equivalent of
   "say $sv".
   These last 2 functions aim to DWYM, irrespective of the type of
   argument that is passed to them. They do not evaluate strings
   in numeric context, but print them as they are.
   YMMV.
