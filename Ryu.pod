...
=head1 FUNCTIONS
   ...
   $str = nv2s($nv);
    ...
    condition (!ryu_SvIOK($nv) && ryu_SvNOK($nv)) is TRUE.
 ...
 $str = d2s($nv);  # Use only when $Config{nvtype} is 'double'.
  ...
  $nv = s2d($str); # string to double
   ...
   Returns an inf or nan as (and when) appropriate.
   Else, if ryu_lln($str) returns a false value, then the s2d()
   call will die with an appropriate error message.
   Otherwise, it returns an NV of the specified 'double' value.
...
=head1 PRINT FUNCTIONS
  ...
  sany(@args); # "s"ay   "any"
   Both print out n2s($arg) for arguments for which the perl API
   function looks_like_number() returns true.
   For all other types of arguments they print out the same as
   print($arg).
   ...
   for the user to ever call perl's print()/say() function if Math::Ryu
   has been loaded.
   Unlike "print", which is generally called without bracketed
   arguments, "pany()" and "sany()" should be called with their
...
=head1 SPRINTF FUNCTION
  ...
  $str = spanyf(@args);
   ...
   Note that the arguments given to spanyf() are enclosed in brackets,
   just as with pany() and sany().
