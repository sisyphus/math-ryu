...
$nv = 1e5000;
cmp_ok(fmtpy_pp(NV2S($nv)), 'eq', $Math::Ryu::pinf, 'fmtpy_pp: +inf stringifies as per perl');
cmp_ok(ryu_refcnt($Math::Ryu::pinfstr), '==', $pinfcount, "2: pinf count ok");
$nv *= -1;
cmp_ok(fmtpy_pp(NV2S($nv)), 'eq', $Math::Ryu::ninf, 'fmtpy_pp: -inf stringifies as per perl');
cmp_ok(ryu_refcnt($Math::Ryu::ninfstr), '==', $ninfcount, "2: ninf count ok");
$nv /= $nv;
cmp_ok(fmtpy_pp(NV2S($nv)), 'eq', $Math::Ryu::nanv, 'fmtpy_pp: nan stringifies as per perl');
