...
if($Config{nvsize} == 8) {
  ...
  *nvtos = \&d2s;
  if(!(2 ** -1075)) {
    cmp_ok(fmtpy(nvtos(2 ** -1075)), '==', 0, "2 ** -1075 is zero");
  }
  else {
    warn "Skipping rendition of 2**-1075 because this dumbarse perl thinks it's non-zero\n";
  }
  cmp_ok(fmtpy(nvtos(2 ** -1074)), 'eq', '5e-324', "2 ** -1074 is 5e-324");
  cmp_ok(fmtpy(nvtos(2 ** -1064)), 'eq', '5.06e-321', "2 ** -1064 is 5.06e-321");
  cmp_ok(fmtpy(nvtos(2 ** -1064 + 2 ** -1074)), 'eq', '5.064e-321', "2 ** -1064 + 2 ** -1074 is 5.064e-321");

}
elsif($Config{nvtype} eq 'long double') {
  $type = 'L';
  *nvtos = \&ld2s;
  cmp_ok(fmtpy(nvtos(2 ** -16446)), '==', 0, "2 ** -16446 is zero");
  cmp_ok(fmtpy(nvtos(2 ** -16445)), 'eq', '4e-4951', "2** -16445 is 4e-4951");
  cmp_ok(fmtpy(nvtos(2 ** -16444)), 'eq', '7e-4951', "2** -16444 is 7e-4951");
  cmp_ok(fmtpy(nvtos(2 ** -16443)), 'eq', '1.5e-4950', "2** -16443 is 1.5e-4950");
  cmp_ok(fmtpy(nvtos(2 ** -16442)), 'eq', '3e-4950', "2** -16442 is 3e-4950");
